...
class CartRepository extends CoreRepository
{
    ...
    public function calculateByCartId(int $id, array $data): array
    {
        ...
        $cart = Cart::with([
            'shop:id,longitude,latitude,tax,price,price_per_km,uuid,logo_img,status',
...
//                        $receiptPrice += !isset($inReceipts[$child->stock_id]) ? $child->price : 0;
		...
		if (data_get($data, 'type') === Order::DELIVERY) {
			...
			$deliveryFee = $helper->getPriceByDistance($km, DeliveryOption::find($data['delivery_option_id']), (float)data_get($data, 'rate', 1));
		}

		if (data_get($data, 'type') === Order::SCHEDULE) {
			$helper      = new Utility;
			$km          = $helper->getDistance($cart->shop->location, data_get($data, 'address'));

			$deliveryFee = $helper->getScheduleOrderDeliveryPrice($cart->shop, $km, (float)data_get($data, 'rate', 1));
		}
        ...
        $tips        = round(data_get($data, 'tips', 0), 2);
        ...
        return [
            ...
            'data'   => [
                ...
                'receipt_discount' => round($receiptDiscount, 2),
