...
class HistoryRepository
{
	...
	public function paginate(array $filter): LengthAwarePaginator
	{
		...
		return Order::filter($filter)
			...
			->when(
				$filter['type'] === 'today',
				fn($query) => $query->where('created_at', '>=', now()->format('Y-m-d 00:00:01'))
			)
	...
	public function cards(array $filter): array
	{
		...
		$transactions = Transaction::with('paymentSystem:id,tag')
			->where(function ($query) {
				$query->where('status', Transaction::STATUS_PAID)
					->orWhereHas('paymentSystem', fn($q) => $q->where('tag', Payment::TAG_CASH));
			})
			->where('created_at', '>=', now()->format('Y-m-d 00:00:01'))
			->get();
		...
		foreach ($transactions as $transaction) {
			...
			if ($transaction?->price && $transaction->paymentSystem?->tag === Payment::TAG_CASH) {
