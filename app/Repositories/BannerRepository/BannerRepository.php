...
//			->when(request()->is('api/v1/rest/*'), fn($q) => $q->whereDoesntHave('shopAdsPackage'))
			...
			->when(data_get($filter, 'shop_ids'), function ($q, $shopIds) {
				$q->whereHas('shops', fn($q) => $q->whereIn('shop_id', $shopIds));
			})
	...
	public function bannerDetails(int $id): mixed
	{
		return $this->model()
			...
			->with([
				'galleries',
				'shops:id,uuid,user_id,status,logo_img,open',
	...
	public function adsPaginate(array $filter): array
	{
		$adsPackages = ShopAdsPackage::with([
			...
			->when(data_get($filter, 'search'), function ($query, $search) {
				$query->whereHas('banner.translation', function ($q) use ($search) {
					$q
						->where('title', 'LIKE', "%$search%")
						->where('locale', $this->language)
						->select('id', 'banner_id', 'locale', 'title');
	...
	{
		$adsPackages = ShopAdsPackage::with([
			'banner.translation' => fn($q) => $q->where('locale', $this->language),
		])
			->where('banner_id', $id)
			->where('active', true)
			->where('status', ShopAdsPackage::APPROVED)
			...
			->whereHas('adsPackage')
			->whereHas('banner',      fn($q) => $q->where('active', true))
			->whereHas('transaction', fn($q) => $q->where('status', Transaction::STATUS_PAID)->orWhereHas('paymentSystem', fn($q) => $q->where('tag', Payment::TAG_CASH)))
			->orderBy('position_page')
			->get();
