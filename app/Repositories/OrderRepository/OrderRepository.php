	...
	public function orderStocksCalculate(array $filter): array
	{
		...
		if (!empty($shop)) {
			...
			if (data_get($filter, 'type') === Order::DELIVERY) {
				...
				$deliveryFee = $helper->getPriceByDistance($km, DeliveryOption::find($filter['delivery_option_id']), (float)data_get($filter, 'rate', 1));
			}

            if (data_get($filter, 'type') === Order::SCHEDULE) {
                $helper      = new Utility;
                $km          = $helper->getDistance($shop->location, data_get($filter, 'address'));

                $deliveryFee = $helper->getScheduleOrderDeliveryPrice($shop, $km, (float)data_get($filter, 'rate', 1));
            }
        ...
        $shopTax     = round($shopTax, 2);
        $serviceFee  = round($serviceFee, 2);
        $tips        = round(data_get($filter, 'tips', 0), 2);
		...
		return [
			...
			'data'   => [
				'stocks'         => $result,
				'total_tax'      => $shopTax,
				'price'          => round($result->sum('price'), 2),
				'total_shop_tax' => $shopTax,
				'total_price'    => $totalPrice,
				'total_discount' => round($result->sum('discount'), 2),
