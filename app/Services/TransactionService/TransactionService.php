...
class TransactionService extends CoreService
{
	...
	public function orderTransaction(int|Model $id, array $data, $class = Order::class): array
	{
        $order = $id;
        ...
        $paymentId = (int)data_get($data, 'payment_sys_id');
		$payment = $this->checkPayment($paymentId, $order, true, $data);
		...
		if (isset($data['tips'])) {
			$transaction = $order->createTransaction([
				'price'              => $transactionId ? $order->tips : $order->total_price,
				...
				'type'	             => $order->transaction?->id ? Transaction::TYPE_TIP : Transaction::TYPE_MODEL,
			]);
		} else {

			$totalPrice = $order->total_price;

			if ($transactionId && !$order->transactions()->where('payment_sys_id', $paymentId)->exists()) {
				$order->createManyTransaction($order->transaction->toArray());
				$totalPrice -= $order->transaction?->price;
			} else if ($order->transaction && $order->transactions->sum('price') < $totalPrice) {
				$order->createManyTransaction($order->transaction->toArray());
				$totalPrice -= $order->transaction?->price;
			}
		...
		if (data_get($payment, 'wallet')) {
			...
			$this->walletHistoryAdd($order->user, $transaction, $order, paymentType: 'withdraw');
	 ...
	 * @throws Throwable
	 */
	public function payDebit(User $user, Order $order): void
    {
		try {
			...
			$this->walletHistoryAdd($user, $transaction, $order, paymentType: 'withdraw');
	...
	private function checkPayment(int $id, $model, bool $isOrder = false, array $data = []): array
	{
		...
		/** @var User $user */
        $userId = data_get($model, 'user_id');

        if (empty($userId)) {
            $userId = data_get($model, 'owner_id');
        }
