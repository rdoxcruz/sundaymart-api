...
class OrderService extends CoreService implements OrderServiceInterface
{
	...
	private function with(): array
	{
		return [
			...
			'shop.translation' => fn($st) => $st->where('locale', $this->language),
			...
			'orderDetails.stock.countable.translation' => fn($ct) => $ct
				->where('locale', $this->language),
			'orderDetails.stock.stockExtras.group.translation' => function ($cgt) {
				$cgt
			...
			'orderDetails.children.stock.countable.translation' => fn($ct) => $ct
				->where('locale', $this->language),
			'orderDetails.children.stock.stockExtras.group.translation' => function ($cgt) {
				$cgt
					->select('id', 'extra_group_id', 'locale', 'title')
					->where('locale', $this->language);
			},
	...
	public function create(array $data): array
	{
		...
		try {
			...
			$order = DB::transaction(function () use ($data, $shop) {
                ...
                $totalPrice = $this->calculateOrder($order, $shop, $data);
				...
				if ($paymentId) {
                    ...
                    if ($payment->tag === 'wallet') {
                        ...
                        (new TransactionService)->walletHistoryAdd($order->user, $transaction, $order, paymentType: 'withdraw');
                    }
	...
	public function update(int $id, array $data): array
	{
		try {
			$order = DB::transaction(function () use ($data, $id) {
                ...
                $isReplace = collect(data_get($data, 'products', []))->contains(function ($item) {
                    return array_key_exists('replace_stock_id', $item);
                });
	 ...
	 * @throws Exception
	 */
	public function calculateOrder(Order $order, ?Shop $shop, array $data): float|int|null
	{
		/** @var Order $order */
		$order = $order->fresh(['orderDetails', 'transactions', 'user', 'coupon', 'shop.subscription.subscription']);
		...
		$order->update([
			...
			'total_discount' => max(round($totalDiscount, 2), 0),
			'tax'            => round($shopTax, 2),
			'tips'           => round($tipsRate, 2),
			'status'		 => $status,
		]);
	...
	public function updateDeliveryMan(?int $orderId, int $deliveryman, ?int $shopId = null): array
	{
		try {
			...
			if ($order->delivery_type === Order::POINT) {
				return [
					'status'  => false,
					'code'    => ResponseError::ERROR_502,
...
//            }
	...
	private function setOrderParams(array $data, Shop $shop): array
	{
		...
		if (data_get($data, 'location') && data_get($data, 'delivery_type') === Order::DELIVERY) {
			...
			$deliveryFee = $helper->getPriceByDistance($km, DeliveryOption::find($data['delivery_option_id']), $rate) / $rate;
		}

		if (data_get($data, 'location') && data_get($data, 'delivery_type') === Order::SCHEDULE) {
			$helper      = new Utility;
			$km          = $helper->getDistance($shop->location, data_get($data, 'location'));
			$deliveryFee = $helper->getScheduleOrderDeliveryPrice($shop, $km, $rate) / $rate;
		}
...
//		}
		...
		return [
			...
			'cash_change'    => data_get($data, 'cash_change'),
			'tax'            => 0,
			'commission_fee' => 0,
			...
			'delivery_option_id'  => data_get($data, 'delivery_option_id'),
