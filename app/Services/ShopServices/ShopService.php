...
class ShopService extends CoreService implements ShopServiceInterface
{
    ...
    private function setShopParams(array $data, ?Shop $shop = null): array
    {
        ...
        return [
            ...
            'user_id'        => $data['user_id'] ?: (!auth('sanctum')->user()->hasRole('admin') ? auth('sanctum')->id() : null),
            'tax'            => $data['tax'] ?? $shop?->tax,
			'email_statuses' => $data['email_statuses'] ?? null,
			'percentage'     => $data['percentage'] ?? $shop?->percentage ?? 0,
            'min_amount'     => $data['min_amount'] ?? $shop?->min_amount ?? 0,
            'phone'          => $data['phone'] ?? null,
            'order_payment'  => $data['order_payment'] ?? Shop::ORDER_PAYMENT_BEFORE,
            'open'           => $data['open'] ?? $shop?->open ?? 0,
            'show_type'      => $data['show_type'] ?? $shop?->show_type ?? 1,
            'status_note'    => $data['status_note'] ?? $shop?->status_note ?? '',
            'price'          => $data['price'] ?? $shop?->price ?? 0,
            'price_per_km'   => $data['price_per_km'] ?? $shop?->price_per_km ?? 0,
            'verify'         => $data['verify'] ?? $shop?->verify ?? 0,
            'delivery_time'  => $deliveryTime,
            'latitude'       => $data['location']['latitude'] ?? $shop?->latitude,
            'longitude'      => $data['location']['longitude'] ?? $shop?->longitude,
        ];
    }
