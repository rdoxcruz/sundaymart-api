 ...
 * @property double|null $r_avg
 * @property double|null $r_count
 * @property double|null $o_count
 * @property double|null $od_count
 ...
 * @property-read DeliveryOption|null $deliveryOption
 * @property-read Collection|DeliveryOption[] $deliveryOptions
...
class Shop extends Model
{
    ...
    public function scopeFilter($query, $filter)
    {
            ...
            DeliveryZone::list()
				...
				->map(function (DeliveryZone $deliveryZone) use ($filter, &$orders) {
                	...
                	if (
						...
						$orders[$deliveryZone->shop_id] = (new Utility)->getPriceByDistance($km, $shop?->deliveryOption, $rate);
                	}
        ...
        $query
            ...
            ->when(data_get($filter, 'ids'), function ($q, $ids) {
                $q->whereIn('id', $ids);
            })
            ...
            ->when(data_get($filter, 'free_delivery'), function (Builder $q) {
                $q->whereHas(
                    'deliveryOption',
                    fn($q) => $q->where([['price', '=', 0], ['price_per_km', '=', 0]])
                );
            })
