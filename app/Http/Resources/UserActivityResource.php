...
class UserActivityResource extends JsonResource
{
    ...
    public function toArray($request): array
    {
        ...
        return [
            ...
            'created_at'    => $this->when($this->created_at, date('Y-m-d H:i:s', strtotime($this->created_at)) . 'Z'),

            /* Relations */
            'user'          => UserResource::make($this->whenLoaded('user')),
            'product'       => $this->when($this->model_type === Product::class, ProductResource::make($this->whenLoaded('model'))),
            'shop'          => $this->when($this->model_type === Shop::class,    ShopResource::make($this->whenLoaded('model'))),
        ];
    }
