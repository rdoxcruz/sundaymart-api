...
class ProductController extends RestBaseController
{
     ...
     */
    public function propertiesByCategory(int $id): mixed
    {
        return $this->restProductRepository->propertiesByCategory($id, request()->all());
    }
    ...
    public function productsSearch(FilterParamsRequest $request): AnonymousResourceCollection
    {
        $products = $this->productRepository->productsSearch(
            $request->merge(['status' => Product::PUBLISHED, 'active' => 1, 'shop_status' => 'approved'])->all(),
        );
