...
class TransactionController extends PaymentBaseController
{
    ...
    public function updateStatus(string $type, int $id, TransactionUpdateRequest $request): JsonResponse
    {
		/** @var Order $model */
		$model = match($type) {
			'parcel-order'  	 => ParcelOrder::with('transactions')->find($id),
			'subscription'  	 => ShopSubscription::with('transactions')->find($id),
			'ads-package', 'ads' => ShopAdsPackage::with('transactions')->find($id),
			'wallet'        	 => Wallet::with(['transactions' => fn($q) => $q->orderBy('id', 'desc')])->find($id),
			default         	 => Order::with('transactions')->find($id),
		};
